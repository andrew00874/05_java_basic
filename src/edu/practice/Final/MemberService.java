...
public class MemberService {
    ...
    public boolean register(String id, String password, String name,
        ...
        if (isValidInput(id,  password, name, email, phone) && isIdExists(id)) {
            Member member = new Member(id, password, name, email, phone, imageUrl);
    ...
    public boolean isIdExists(String id) {
        ...
        List<Member> members = fileManager.loadAllMembers();
    ...
    private boolean isValidInput(String id, String password, String name,
        ...
        // 전화번호 10자 이상 확인
        // 각 검증 실패시 에러 메시지 출력
        if (id == null || password == null || name == null || email == null || phone == null) {
            return false;
        }
        if (id.trim().length() < 3 || password.trim().length() < 4){
            return false;
        }
        if (!email.contains("@")){
            return false;
