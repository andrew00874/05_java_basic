...
public class VariableEx1 {
    public static void main(String[] args) {
        ...
        * */
        ...
        //          (이미 값이 존재하는 변수이름에 대입하면 --> 덮어쓰기가 됨)

        isTrue = true; // null 빈 공간값에 true 라는 명칭으로 덮어쓰기

        // sout + tab = System.out.println() 자동완성
        // so   + tab = System.out.printf("") 자동완성
        System.out.println("isTrue : " + isTrue); // true
        isTrue = false;
        System.out.println("isTrue : " + isTrue); // false로 덮어쓰기 되었음

        //변수선언 + 초기화 + 값의 범위 확인
        int number2 = 2100000000; // 21억 오류 X
        //long number3 = 2200000000; // int 22억 오류 O
        long number3 = 2200000000L; // 숫자 뒤에 L 을 붙여서 선언해야 오류가 발생하지 않는다.
        ...
        * 리터럴과 값의 차이
        *
        * 리터럴   : 코딩에서 직접적으로 작성된 고정된 값
        * 값      : 변수나 표현식이 실제로 가지고 있는 데이터
        *
        * int a = 100;               // 리터럴이자 값
        ...
        *           그런데 nt2 에 2배가량이 넘는 long 데이터를 담아 넣으려 하기 때문에
        *           바구니 크기에 비하여 데이터가 크다는 문제가 발생
        *
        *           값을 억지로 넣을 수는 있지만 값이 왜곡되어 변수이름 바구니에 담김
        *           => 강제 형변환
        *               숫자나 문자 구문 앞에 바구니 자료형을 소괄호로 작성하면 형 변환
        *
        * */
        int nt1 = 100;
        int nt2 = (int)10000000000L; // long 데이터가 int로 강제 형변환, 데이터 변질

        long lng1 = 200;            // int 리터럴을 2배 가량의 메모리를 차지하는 Long
                                    // 큰 바구니에 작은 데이터를 담는 것은 문제가 되지 않음
                                    // long 과 같이 리터럴에 L을 작성하는 경우에는
                                    // 자료형 타입에도 Long을 작성하는 사소한 문제를 발생할 수 있음
