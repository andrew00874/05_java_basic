    ...
    public static class FileEx1 {
        ...
        * 유틸리티(Utility): 편리한 기능들을 모아놓은 클래스
        ...
        public void legacyMethod(){
            ...
            // copy() move() readAllText()  파일복사 파일이동 파일내모든택스트읽기 기능 등 다수 없음
        ...
        public void factoryMethod(){
            ...
            //                    User\\user\\file.text 와 같은 형태의 슬래시가 알아서 생성

            // Linux or Mac 에서 : Users/user/file.text 와 같은 형태의 슬래시가 알아서 생성
        ...
        * Files.copy(Path.of("원본파일경로/원본파일이름.확장자이름"),Path.of("복사해놓을파일경로/복사본파일이름/확장자이름").StandardCopyOption.REPLACE_EXIST
        *                                                                                                                   파일 복사할 때 덮어쓰기 형태로 복사
        * Files.move(Path.of("원본파일경로/원본파일이름.확장자이름"),Path.of("복사해놓을파일경로/복사본파일이름/확장자이름").StandardCopyOption.REPLACE_EXIST
        * Files.move(Path.of("원본파일경로/원본파일이름.확장자이름"),Path.of("복사해놓을파일경로/복사본파일이름/확장자이름").StandardCopyOption.REPLACE_EXIST
        *                                                                                                                   파일 이동할 때 덮어쓰기 형태로 이동
        * Files.delete(Path.of("원본파일경로/원본파일이름.확장자이름"),Path.of("복사해놓을파일경로/복사본파일이름/확장자이름").StandardCopyOption.REPLACE_EXIST
        *
        ...
        *
        * Path.getParent() = 상위 디렉터리까지의 모든 경로를 가져오는 기능
        *
        * Path.getFileName() = 파일경로 이외에 [파일명칭.확장자이름] 만 가지고 오는 기능
        *
        * Path.toAbsolutePath() = 절대 경로 반환
        *
        * Path.getNameCount() = 파일위치에 도달하기 까지 몇개의 / 를 거치는가 ? 경로 갯수 새는 것
        * int nameCount = Path.getNameCount(); //파일 이름 전까지 슬래시 갯수로 폴더 갯수 가져옴
        ...
        *  파일에서는 BufferedReader / BufferedWridter 로 파일읽기 쓰기를 구현
        * */
