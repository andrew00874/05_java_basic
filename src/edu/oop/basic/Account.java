...
//     속성 (값) - DB 컬럼명칭과 맞추어 상호작용하여 데이터를 주고 받을 명칭
    ...
    /* 단축키 Alt + insert 를 클릭하면 생성자 getter setter 를 만들어주는 기능 실행 가능
        메서드 (기능) - 생성자
                      생성자 - 기본
                      생성자 - 필수
...
//    메서드 (기능) - setter : html에서 사용자에게 전달받은 값을 자바의 변수이름 내부에 저장하는 역할
    ...
    * if(계좌잔액 < 0){
    *       // 잔액은 음수가 될 수 없습니다.
    *       return; // 계좌 잔액에일치하지 않는 데이터를 돌려보내기
    *  }
    *
    ...
    *   this.name = html 에서 데이터 전달받아올 변수이름;
    * }
    *
    * setName 은 private String name 변수명칭에 데이터를 저장하기 위한 메서드
    * this.name = 위에서 작성한 private String name 가리킴
    *
    * this.name = name 에서 name 은 String name 에서 가져온 변수명칭을 가리킴
    *
    * (String name) 매개변수 내부에 존재하는 변수이름은 private에서 작성한 변수이름과 다르게
    * 작성해도 상관없지만, private으로 작성한 속성명칭이 많아지면 변수이름들을 관리하는데 문제가 발생
    * 할 수 있기 때문에 속성명칭과 매개변수 내부에 작성한 변수 명칭을 동일하게 작성해주는 것이 바람직함.
    *
    ...
    *           return 에서 작성되어 있는 변수명칭의 자료형은 public 옆에 존재하는 위치에 작성
    *
    *           public 리턴할 변수이름의 자료형 get변수이름() {
    ...
    *
    * @param pw         : 클라이언트가 작성한 비밀번호를 가지고 있는 매개변수
    ...
    * */
    ...
    public void withdraw(String pw, long amount) {
        // 자바에서
        // - 기본 자료형 비교는 ==
        // - 객체(참조형) 비교는 A.equals(B) 대표적인예) String 문자열 비교할 경우 equals 사용

        // 현재 계좌 비밀번호 (password) 와
        // 전달받은 비밀번호(pw) 가 같을경우 true/다르면 false
