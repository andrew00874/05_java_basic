...
public interface 인터페이스 {
    /**
     * 멤버 인스턴스변수 속성 을 작성할 때 임의의 공간이 설정된 속성이 아닌 데이터가 들어있는 상수이름만 생성 가능
     private 불가!!!
     String name = "김개발"; 리터럴 형태만 가능

     인터페이스에서 작성하는 모든 ~ 필드 명칭은 자동으로 public static final 로 처리됨
    ...
    // static으로 작성해야하는 데이터는 나중에 설정파일 이라는 곳에 한 번에 작성할 예정
