...
public class ListService {
     ...
     */
    public void method1(){
        int[] a = {1,2,3}; // int 이기 때문에 숫자 형태만 가능
        ...
        // 1. .add(E e); 목록에서 마지막에 e 에 해당하는 데이터를 추가
    ...
    public void method2(){
        ...
        //     ↓<> 필수로 String 형태만 담을 수 있도록 설정    ↓<> 에 String 을 작성해도되고, 안해도됨
        ...
        //              strList에서 필요한 타입은 String인데 int 타입이 제공되어서 발생하는 문제
        ...
        //System.out.println("strList 3번 째: " + strList.get(3)); ArrayList 데이터가 3개만 있기에 문제 발생
        ...
        // 4. String toString() : List 인터페이스를 만들면서 toString 을추가로 작성하는 것이 번거로움을 느낌
        ...
        System.out.println(strList.indexOf("보쌈"));   //  3번 째 자리에 위치
        System.out.println(strList.indexOf("막국수")); //  1번 째 자리에 위치
        System.out.println(strList.indexOf("콩국수")); // -1 strList 존재하지 않음
        ...
        // index 번째 요소를 제거
        //  -> 제거된 요소가 반환됨
        String target = strList.remove(0); // 0 번째 인덱스 요소를 제거하겠다.
        // target 에는 strList 에서 제거되기 전에 제거될 데이터의 값이 담겨진 후 strList 에서는 해당 데이터가 제거됨
    ...
    *                  실행 취소 기능 (앞뒤로 이동)
    *                  대기열 관리 (앞에서 뺴고 뒤에서 넣기)
    * 대부분의 경우에는 ArrayList를 주로 사용
    * LinkedList의 경우에는 중간에 삽입/ 삭제가많을 경우에만 사용
    *
    * */
